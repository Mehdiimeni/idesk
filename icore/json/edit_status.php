...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        if (isset($_POST['table_set'], $_POST['operation'], $_POST['id'])) {
            ...
            $sqlQuery = "INSERT INTO status (`part_id`, `part_name`, `status_name`, `admin_id`, `rbac_id`, `status_description`) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $db->prepare($sqlQuery);
            if ($stmt === false) {
                throw new Exception(_lang['query_preparation_failed']);
            }

            $stmt->bind_param("isssis", $id, $table_set, $operation, $_SESSION["admin_id"], $_SESSION["rbac_id"], $description);
