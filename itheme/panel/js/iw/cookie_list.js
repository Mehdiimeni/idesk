...
const setupSelectAll = (selectAllId, checkboxClass) => {
    ...
    // Only set up if the selectAll element exists
    if (!selectAll) {
        console.warn(`Element with ID '${selectAllId}' not found`);
        return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setupSelectAll('selectAllUnits', 'unit-checkbox');
    setupSelectAll('selectAllTypes', 'type-checkbox');
    setupSelectAll('selectAllStatuses', 'status-checkbox');
    
    // مدیریت چک‌باکس‌های تکی
    const checkboxes = document.querySelectorAll('.form-check-input:not([id^="selectAll"])');
    checkboxes.forEach(checkbox => {
        ...
        checkbox.addEventListener('change', function() {
            ...
            const sectionCheckboxes = document.querySelectorAll(`.${this.className.replace('form-check-input', '').trim()}`);
            const selectAllId = `selectAll${this.className.replace('-checkbox', '').replace('form-check-input', '').trim().charAt(0).toUpperCase() + this.className.replace('-checkbox', '').replace('form-check-input', '').trim().slice(1)}`;
            const selectAll = document.getElementById(selectAllId);
