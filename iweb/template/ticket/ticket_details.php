                        ...
                        <div class="card border shadow-sm">
                            <div class="card-body">
                                ...
                                <?php while ($commentDetail = $allComments->fetch_assoc()) { ?>
                                    ...
                                    $isCommentOwner = !empty($commentDetail['user_id']) &&
                                        (int) $commentDetail['user_id'] === (int) $_SESSION['user_id'];

                                    // کامنت غیرفعال فقط برای سازنده همان کامنت قابل مشاهده است.
                                    if (!$commentIsActive && !$isCommentOwner) {
                                        continue;
                                    }

                                    $commentInactiveClass = $commentIsActive ? '' : 'comment-inactive';
                                    ...
                                    <div
                                        class="card comment-card mb-3 <?php echo $commentBorderClass . ' ' . $commentInactiveClass; ?>">
                                        <div class="card-body">
                                                ...
                                                <div class="d-flex align-items-center gap-2">
                                                    <?php if (!$commentIsActive) { ?>
                                                        <span class="badge bg-secondary">
                                                            <?php echo _lang['deactivated'] ?? 'غیرفعال شده'; ?>
                                                        </span>
                                                    <?php } ?>
                                            ...
                                            <?php if ($commentIsActive && $isCommentOwner) { ?>
                                                <div class="mt-3 text-end">
                                                    <form method="post" 
                                                        action="./tickets?ticket_id=<?php echo $_GET['ticket_id']; ?>"
                                                        class="d-inline"
                                                        onsubmit="return confirm('<?php echo _lang['deactivate_comment_confirm'] ?? 'آیا از غیرفعال‌سازی این کامنت مطمئن هستید؟'; ?>');">
                                                        <input type="hidden" name="deactivate_comment_id"
                                                            value="<?php echo (int) $commentDetail['id']; ?>">
                                                        <button type="submit" name="deactivate_comment"
                                                            class="btn btn-sm btn-outline-secondary rounded-pill px-3">
                                                            <i class="mdi mdi-eye-off-outline"></i>
                                                            <?php echo _lang['deactivate'] ?? 'غیرفعال کردن'; ?>
                                                        </button>
                                                    </form>
                                                </div>
                                            <?php } ?>

                                            <?php if ($commentIsActive && $rbacClass->checkPermissionOperationByName('reply_comment_operation', 'u')) { ?>
                                                <div class="mt-3 text-end">
                                                    <button type="button"
                                            ...
                                            while ($replyDetail = $replyComments->fetch_assoc()) {
                                                ...
                                                $isReplyOwner = !empty($replyDetail['user_id']) &&
                                                    (int) $replyDetail['user_id'] === (int) $_SESSION['user_id'];

                                                // پاسخ غیرفعال فقط برای سازنده همان پاسخ قابل مشاهده است.
                                                if (!$replyIsActive && !$isReplyOwner) {
                                                    continue;
                                                }

                                                $replyInactiveClass = $replyIsActive ? '' : 'comment-inactive';
                                                ...
                                                <div class="ms-md-5 ms-3 mt-3">
                                                    <div
                                                        class="card comment-card reply-card <?php echo $replyBorderClass . ' ' . $replyInactiveClass; ?>">
                                                        <div class="card-body py-3">
                                                            ...
                                                            <?php if ($replyIsActive && $isReplyOwner) { ?>
                                                                <div class="mt-3 text-end">
                                                                    <form method="post"
                                                                        action="./tickets?ticket_id=<?php echo $_GET['ticket_id']; ?>"
                                                                        class="d-inline"
                                                                        onsubmit="return confirm('<?php echo _lang['deactivate_comment_confirm'] ?? 'آیا از غیرفعال‌سازی این پاسخ مطمئن هستید؟'; ?>');">
                                                                        <input type="hidden" name="deactivate_comment_id"
                            ...
                            <div class="modal-dialog modal-dialog-centered">
                                <form validate class="modal-content"
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <textarea class="form-control" name="comment_text" id="modal_comment_text" rows="4"
                                            required placeholder="<?php echo _lang['your_answer']; ?>"></textarea>
                                    </div>
